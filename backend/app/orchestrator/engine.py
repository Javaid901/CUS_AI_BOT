...
async def _execute_plan(
    ...
    if action == "student_service":
        ...
        if not student_session:
            state.student_gate = {"stage": "auth", "family": family}
...
def _admit_card_events(
    ...
    """
    from app.student.gate import admit_card_detail_event, admit_card_semesters_event
    ...
    if semester is not None:
        ...
        if data is None:
            ...
            yield admit_card_detail_event(semester, data)
        return
...
def _exam_form_events(
    ...
    Each form is identified by (exam_type, semester). Resolution order:
      1. chip id `exam_form{type}{sem}` — clicked picker option
      2. entities.semester — typed request (falls back to the Regular form)
      3. otherwise the picker (one chip per provisioned form)
    A selected semester/type with no form gets a plain safe message — never an
    error, never a hint about other students' data.
    """
    from app.student.gate import exam_form_detail_event, exam_form_picker_event
    ...
    semester: int | None = None
    m = _EXAM_OPTION_ID.match((message or "").strip().lower())
    if m:
