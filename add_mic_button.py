...
# Find the input-wrap div and insert mic button after textarea and before send button
textarea_start = content.find('<textarea rows="1"')
if textarea_start < 0:
...
send_start = content.find('<button class="send"', textarea_end)
if send_start < 0:
    print('ERROR: send button not found after textarea')
    exit(1)

# The mic button to insert (from the previous working implementation)
...
print(f'Total length: {len(new_content)}')
PYEOF